...
def _group_window_chunk_worker(args):
...
class MagentGroupWindowConstructor:
    ...
    def _process_parallel(self, state_last, grouping, alive_last):
        ...
        try:
            ...
            with ProcessPoolExecutor(max_workers=n_workers) as executor:
                chunk_results = list(
                    executor.map(_group_window_chunk_worker, args)
                )
    ...
    def _process_single(self, st, grp, alv):
        """Process a single batch element into per-group window crops.
        ...
        Returns:
            ...
            - group_result: (n_groups, sel_C, K, K) or (n_groups, K, K, sel_C)
        ...
        if self.channel_first:
            ...
            group_result = np.zeros(
                (self.n_groups, len(self.selected_channels), K, K), dtype=np.float16
            )
        else:
            # st: (H, W, C)
            H, W = st.shape[0], st.shape[1]
            group_result = np.zeros(
                (self.n_groups, K, K, len(self.selected_channels)), dtype=np.float16
            )
