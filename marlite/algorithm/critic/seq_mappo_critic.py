"""SeqMAPPOCritic — state-value critic that processes full state sequence.

Follows the same pattern as SeqQMixer (``critic/seq_qmix_mixer.py``): state
features are extracted per timestep, passed through a sequence model
(RNN, Conv1D, or Attention), and the final hidden state is mapped to a
scalar value V(s_{T-1}).
"""
...
class SeqMAPPOCritic(Critic):
    """Value network that processes the full state sequence.

    Parameters
    ----------
    base_model_config : ModelConfig
        Model that maps the sequence model's output to a scalar value.
    feature_extractor_config : ModelConfig
        Per-timestep feature extractor.
    seq_model_config : ModelConfig
        Sequence model (RNN, Conv1D, or Attention) for temporal processing.
    """
    ...
    def forward(
        ...
        Returns:
            ``{"v": (B, 1)}`` — value of the **last** timestep, conditioned
            on the full state sequence.
        """
        bs = states.shape[0]
        ts = states.shape[1]

        # Flatten batch × time so the FE processes each timestep independently.
        # ``states.shape[2:]`` is empty for 3D ``(B, T, F)``
        # and ``(N, F)`` for 4D ``(B, T, N, F)``.
