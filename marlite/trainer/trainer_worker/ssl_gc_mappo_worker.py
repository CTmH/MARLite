...
class SSLGroupConsensusMAPPOWorker(OnPolicyWorker):
    ...
    def train_step(self, batch: Dict[str, Any]) -> Dict[str, float]:
        """Preserve the original combined PPO+SSL worker operation."""
        return self._joint_train_step(batch)

    def _joint_train_step(self, batch: Dict[str, Any]) -> Dict[str, float]:
        # ``epoch`` in a legacy batch remains accepted.  New callers send it
        # separately through SET_TRAINING_EPOCH.
        current_epoch = batch.get("epoch", self.current_training_epoch)
