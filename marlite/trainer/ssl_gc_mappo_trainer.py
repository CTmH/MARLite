        ...
        for epoch in range(times):
            ...
            with tqdm(
                ...
                for batch in dataloader:
                    ...
                    self._accumulate_metrics(totals, result)
                    total_batches += 1

                    bs = batch["states"].shape[0]
                    pbar.update(bs)

        metrics = self._average_metrics(totals, total_batches)
        logging.info(
            "  Iter %d [joint]: RL Loss %.4f, SSL Loss %.4f, Approx KL %.6f, "
            "Clip Fraction %.4f, updates %d",
            self.current_epoch,
            metrics.get("rl_loss", 0.0),
            metrics.get("ssl_loss", 0.0),
            metrics.get("approx_kl", 0.0),
            metrics.get("clip_fraction", 0.0),
            total_batches,
        )
        return metrics
