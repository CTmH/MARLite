...
class FullConnGraphBuilder(GraphBuilder):
    ...
    def forward(self, state: np.ndarray) -> Tuple[ndarray, List[ndarray]]:
        ...
        if self.add_self_loop:
            batch_adj_matrices = np.ones((bs, num_nodes, num_nodes),dtype=np.int64)
            edge_indices = edge_indices + [[i, i] for i in range(num_nodes)]
        else:
            batch_adj_matrices = np.ones((bs, num_nodes, num_nodes),dtype=np.int64) - np.eye(num_nodes, dtype=np.int64)
